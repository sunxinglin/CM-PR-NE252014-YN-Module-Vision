...
namespace VisDummy.Protocols.模组检测.Middlewares.Common
{
    public class MaintainMiddleware : IWorkMiddleware<ScanContext>
    {
        ...
        private readonly 模组检测Scanner _scanner;

        public MaintainMiddleware(ILogger<MaintainMiddleware> logger, IOptionsMonitor<S7ScanOpt> scanOptsMonitor, IMediator mediator, 模组检测Scanner scanner)
        {
            _logger = logger;
