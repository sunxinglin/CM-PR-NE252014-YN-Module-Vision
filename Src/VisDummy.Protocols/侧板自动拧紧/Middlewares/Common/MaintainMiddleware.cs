...
namespace VisDummy.Protocols.侧板自动拧紧.Middlewares.Common
{
    public class MaintainMiddleware : IWorkMiddleware<ScanContext>
    {
        ...
        private readonly 侧板自动拧紧Scanner _scanner;

        public MaintainMiddleware(ILogger<MaintainMiddleware> logger, IOptionsMonitor<S7ScanOpt> scanOptsMonitor, IMediator mediator, 侧板自动拧紧Scanner scanner)
        {
            _logger = logger;
