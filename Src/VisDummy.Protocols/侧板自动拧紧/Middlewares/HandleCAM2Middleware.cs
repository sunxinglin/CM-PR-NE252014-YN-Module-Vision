...
namespace VisDummy.Protocols.侧板自动拧紧.Middlewares
{
	public class HandleCAM2Middleware(
		...
		ILogger<HandlePlcRequestMiddlewareBase<DevMsg_CAM2, MstMsg_CAM2, StationArgs, StationOkWrap_侧板自动拧紧, StationErrWrap_侧板自动拧紧>> logger,
		IMediator mediator) :
			...
			>(flusher, logger, mediator)
	{
		...
		public override bool HasAck(MstMsg_CAM2 p) => p.Flag.Ready;
		...
		protected override async Task<FSharpResult<StationOkWrap_侧板自动拧紧, StationErrWrap_侧板自动拧紧>> HandleArgsAsync(StationArgs args)
		{
			...
			var res = from r1 in visParams.SetTriggerGlobalParams()
					  ...
					  from r2 in visproc.机器人2相机ProcAsync(args, camModelInput)
					  select r2;
			return await res;
		}

		protected override async Task HandleErrAsync(MstMsg_CAM2 pending, StationErrWrap_侧板自动拧紧 err)
		{
        ...
        protected override async Task HandleOkAsync(MstMsg_CAM2 pending, StationOkWrap_侧板自动拧紧 descriptions)
        {
