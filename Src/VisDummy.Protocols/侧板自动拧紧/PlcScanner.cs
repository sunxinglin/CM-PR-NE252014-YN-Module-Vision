...
{
    public class 侧板自动拧紧Scanner : S7PlcScanner<DevMsg, MstMsg>
    {
        private readonly IMediator _mediator;

        public 侧板自动拧紧Scanner(IMediator mediator, IServiceScopeFactory ssf, ILogger<S7PlcScanner<DevMsg, MstMsg>> logger, IOptionsMonitor<S7ScanOpt> scanOpts, S7PlcMgr plcMgr)
        ...
        public override string ScanName => PlcNames.PLCNAME_侧板自动拧紧;
        ...
        public override async Task<MstMsg> GetMstMsgAsync(S7ScanOpt scanOpt)
        {
            ...
            var ss = Marshal.SizeOf<MstMsg_CAM1>();
            if (obj == null)
            {
