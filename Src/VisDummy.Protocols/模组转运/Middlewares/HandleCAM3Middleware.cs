...
namespace VisDummy.Protocols.模组转运.Middlewares
{
	public class HandleCAM3Middleware(
		...
		ILogger<HandlePlcRequestMiddlewareBase<DevMsg_CAM3, MstMsg_CAM3, StationArgs, StationOkWrap_模组转运, StationErrWrap_模组转运>> logger,
		IMediator mediator) :
			...
			>(flusher, logger, mediator)
	{
        ...
        protected override async Task HandleErrAsync(MstMsg_CAM3 pending, StationErrWrap_模组转运 err)
        {
        ...
        protected override async Task HandleOkAsync(MstMsg_CAM3 pending, StationOkWrap_模组转运 descriptions)
        {
