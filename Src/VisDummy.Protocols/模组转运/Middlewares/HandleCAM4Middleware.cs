...
{
	public class HandleCAM4Middleware(
		模组转运Flusher flusher,
		IVisProc visproc,
		IVisParams visParams,
		ILogger<HandlePlcRequestMiddlewareBase<DevMsg_CAM4, MstMsg_CAM4, StationArgs, StationOkWrap_模组转运, StationErrWrap_模组转运>> logger,
		IMediator mediator) :
			...
			>(flusher, logger, mediator)
	{
		...
		public override bool HasAck(MstMsg_CAM4 p) => p.Ready;


		public override bool HasReq(DevMsg_CAM4 i) => i.Flag.Req;

		public override DevMsg_CAM4 RefIncoming(ScanContext ctx) => ctx.DevMsg.CAM4;


		public override MstMsg_CAM4 RefPending(ScanContext ctx) => ctx.MstMsg.CAM4;
		...
		protected override async Task<FSharpResult<StationOkWrap_模组转运, StationErrWrap_模组转运>> HandleArgsAsync(StationArgs args)
		{
			...
			var res = from r1 in visParams.SetTriggerGlobalParams()
					  ...
					  from r2 in visproc.模组转运机器人1相机ProcAsync(args, camModelInput)
					  select r2;
			return await res;
		}

        protected override async Task HandleErrAsync(MstMsg_CAM4 pending, StationErrWrap_模组转运 err)
        {
        ...
        protected override async Task HandleOkAsync(MstMsg_CAM4 pending, StationOkWrap_模组转运 descriptions)
        {
        ...
        protected override async Task ResetAckAsync(MstMsg_CAM4 pending)
		{
		...
		protected override FSharpResult<StationArgs, string> TryParseArgs(DevMsg_CAM4 incoming)
		{
		...
		{
			DevMsg_CAM4 incoming = RefIncoming(context);
			MstMsg_CAM4 pending = RefPending(context);
			bool skipNext = false;
			try
			{
