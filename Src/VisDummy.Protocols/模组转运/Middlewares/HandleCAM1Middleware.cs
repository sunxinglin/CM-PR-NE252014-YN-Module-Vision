...
namespace VisDummy.Protocols.模组转运.Middlewares
{
	public class HandleCAM1Middleware(
		...
		ILogger<HandlePlcRequestMiddlewareBase<DevMsg_CAM1, MstMsg_CAM1, StationArgs, StationOkWrap_模组转运, StationErrWrap_模组转运>> logger,
		IMediator mediator) :
			...
			>(flusher, logger, mediator)
	{
		...
		public override bool HasAck(MstMsg_CAM1 p) => p.Ready;
        ...
        protected override async Task HandleErrAsync(MstMsg_CAM1 pending, StationErrWrap_模组转运 err)
        {
        ...
        protected override async Task HandleOkAsync(MstMsg_CAM1 pending, StationOkWrap_模组转运 descriptions)
        {
