...
namespace VisDummy.Protocols.垫片检测.Middlewares.Common
{
    public class MaintainMiddleware : IWorkMiddleware<ScanContext>
    {
        ...
        private readonly 垫片检测Scanner _scanner;

        public MaintainMiddleware(ILogger<MaintainMiddleware> logger, IOptionsMonitor<S7ScanOpt> scanOptsMonitor, IMediator mediator, 垫片检测Scanner scanner)
        {
            _logger = logger;
