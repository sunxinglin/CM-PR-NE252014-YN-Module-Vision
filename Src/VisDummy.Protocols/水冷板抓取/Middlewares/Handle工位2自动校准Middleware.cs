...
{
    public class Handle工位2自动校准Middleware(
        PlcCtrlFlusher flusher,
        IVisProc visproc,
        ILogger<Handle工位2自动校准Middleware> logger,
        IMediator mediator
        ) : HandlePlcRequestMiddlewareBase<DevMsg_自动校准, MstMsg_自动校准, StationArgs, 自动校准OkWrap_水冷板抓取, 自动校准ErrWrap_水冷板抓取>(flusher, logger, mediator)
    {
        ...
        public override string ProcName => "机器人2自动校准";

        public override bool HasAck(MstMsg_自动校准 p) => p.Ack;

        public override bool HasReq(DevMsg_自动校准 i) => i.Trigger;

        public override DevMsg_自动校准 RefIncoming(ScanContext ctx) => ctx.DevMsg.机器人2自动校准;

        public override MstMsg_自动校准 RefPending(ScanContext ctx) => ctx.MstMsg.机器人2自动校准;
        ...
        protected override async Task<FSharpResult<自动校准OkWrap_水冷板抓取, 自动校准ErrWrap_水冷板抓取>> HandleArgsAsync(StationArgs args)
        {
            ...
            var res =
                ...
                from r2 in visproc.水冷板抓取自动校准2ProcAsync(args)
                select r2;
            return await res;
        }
