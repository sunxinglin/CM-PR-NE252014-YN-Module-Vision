...
{
    public class Handle工位2定位引导Middleware(
        PlcCtrlFlusher flusher,
        IVisProc visproc,
        ILogger<Handle工位2定位引导Middleware> logger,
        IMediator mediator
        ) : HandlePlcRequestMiddlewareBase<DevMsg_定位引导, MstMsg_定位引导, StationArgs, 定位引导OkWrap_水冷板抓取, 定位引导ErrWrap_水冷板抓取>(flusher, logger, mediator)
    {
        ...
        public override string ProcName => "机器人2定位引导";

        public override bool HasAck(MstMsg_定位引导 p) => p.Ack;

        public override bool HasReq(DevMsg_定位引导 i) => i.Trigger;

        public override DevMsg_定位引导 RefIncoming(ScanContext ctx) => ctx.DevMsg.机器人2定位引导;

        public override MstMsg_定位引导 RefPending(ScanContext ctx) => ctx.MstMsg.机器人2定位引导;
        ...
        protected override async Task<FSharpResult<定位引导OkWrap_水冷板抓取, 定位引导ErrWrap_水冷板抓取>> HandleArgsAsync(StationArgs args)
        {
            ...
            var res =
                ...
                from r2 in visproc.水冷板抓取定位引导2ProcAsync(args)
                select r2;
            return await res;
        }
