...
{
    public class Handle工位1校验Middleware(
        PlcCtrlFlusher flusher,
        IVisProc visproc,
        ILogger<Handle工位1校验Middleware> logger,
        IMediator mediator
        ) : HandlePlcRequestMiddlewareBase<DevMsg_校验, MstMsg_校验, StationArgs, 校验OkWrap_水冷板抓取, 校验ErrWrap_水冷板抓取>(flusher, logger, mediator)
    {
        ...
        public override string ProcName => "机器人1校验";

        public override bool HasAck(MstMsg_校验 p) => p.Ack;

        public override bool HasReq(DevMsg_校验 i) => i.Trigger;

        public override DevMsg_校验 RefIncoming(ScanContext ctx) => ctx.DevMsg.机器人1校验;

        public override MstMsg_校验 RefPending(ScanContext ctx) => ctx.MstMsg.机器人1校验;
        ...
        protected override async Task<FSharpResult<校验OkWrap_水冷板抓取, 校验ErrWrap_水冷板抓取>> HandleArgsAsync(StationArgs args)
        {
            ...
            var res =
                ...
                from r2 in visproc.水冷板抓取校验1ProcAsync(args)
                select r2;
            return await res;
        }
